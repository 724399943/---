    ...
    <div class="content">
     ...
     <div class="postWarp">
          <div class="pcontent">
               ...
               <div class="p-postmsg">
                  <!-- <div class="texa" id="editor">
                  </div> -->
                    <mavon-editor id="editor" @change="$change" @imgAdd="$imgAdd" @imgDel="$imgDel"></mavon-editor>
               </div>
               <div class="p-fun">
                  ...
                  <a href="javascript:;" class="submit" @click="toPost">发布</a>
               </div>
          </div>
     </div>
    </div>
...
export default {
  ...
  methods: {
      toPost(){
          ...
          this.$http.post('/Shop/Post/toPost', this.dataJson, {emulateJSON:true}).then(function(response){
              var returnData = response['data'];
              if ( returnData['status'] == '200000' ) {
                  this.$store.commit('alert', {show:true,text:'发帖成功'});
                  this.$router.push({name:'decorateACircle'});
              } else {
