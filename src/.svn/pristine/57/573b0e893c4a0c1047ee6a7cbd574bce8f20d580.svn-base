	  			...
	  			that.$http.post('/Shop/Login/register', requestData, {emulateJSON:true}).then(function(response){
					...
					if( returnData['status'] == '200000' ){
						...
						if ( this.wechatAgent == false ) {
							window.location.href = "mitchell://login?user_id="+ returnData['data']['user']['id'] +'&session_id='+ returnData['data']['session_id'] +'&headimgurl='+ returnData['data']['user']['headimgurl'] + '&nickname=' +returnData['data']['user']['nickname'];
						}
						this.$router.push('/');
					}else{
						that.processTips(1, returnData['message']);
