			...
			<div class="news_wrap">
				<div class="system_news">
					<router-link to="/systemMessage">
						...
						<div class="s_new_m">
							<p class="stt"><span>系统消息</span><em v-if="system.add_time != null">{{system.add_time | filterTime}}</em></p>
...
export default {
	...
	methods: {
		...
		//获得系统未读信息数目
		getSystemMsgCount(){
			this.$http.post('/Shop/Message/systemMessage', {}, {emulateJSON:true}).then(function(response){
				if(  response.data.status == "200000" ){
					var total = 0;
					var returnList = response.data.data.list;
					for( var i=0; i<returnList.length; i++){
						var data = returnList[i];
						if( data.is_read == '0')
							total++;
					}
					this.systemCount = total;
				}
			});
		},
		loadUserChat(){
            ...
            if ( that.loadData == true ) {
                ...
                that.$http.post('/Shop/Chat/chatHistory', that.dataJson, {emulateJSON:true}).then(function(response){
                    var returnData = response['data'],
                    	returnList = returnData['data']['list'];
                    if ( returnList.length ) {
                    	for (var i = 0; i < returnList.length; i++) {
                    		returnList[i]['count'] = 0;
                    	}
                        that.list = ( that.list.length ) ? that.list.concat(returnList) : returnList;
                        that.from = 'yjwy' + returnData['data']['user']['id'];
                        if ( this.wechatAgent === true ) {
                        	that.$store.commit('imlogin', {userId : that.from, passWord: that.from});
                        }
                        that.initMessage();
                    } else {
                        if ( that.list.length ) {
