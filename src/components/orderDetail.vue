  	...
  	<div class="content">
		...
		<div class="main">
			<div class="yyodetail">
				...
				<div class="yytotal">实付款： <span>{{orderData.real_pay}}</span></div>
				<div class="yyoddelbox">
					...
					<div class="yotbox" v-else-if="orderData.status == 2">
						<router-link to="/returnsAfterSales" class="yyoddel">申请退货</router-link>
						<a href="javascript:;" class="yyoddel" @click="postponeReceived(orderData.order_sn)" v-if="orderData['is_postpone'] == '0'">延期收货</a>
						<a href="javascript:;" class="yyoddel" v-else>已延期</a>
					...
					<div class="yotbox" v-else-if="orderData.status == 3">
						<router-link :to="{name: 'selEvaluateGoods', query: {'order_sn': orderData['order_sn']}}" class="yyoddel">
		        			评价
		        		</router-link>
					</div>
