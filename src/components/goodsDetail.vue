	...
	methods: {
		...
		addToCart : function(){
			...
			if( this.isSelect == 0 ){
				...
				// console.log(this.chooseDescription);
				if ( (!this.attrData[0] && !this.attrData[1]  || this.attrData[0] && !this.attrData[1]  || !this.attrData[0] && this.attrData[1]) && this.shoppingCartJson.sale_id == '0' && this.shoppingCartJson.daily_id == '0' && this.attrNameInfo.length == '2') {
					this.$store.commit('alert',{show:true,text:this.chooseDescription});
					return;
				}else if( !this.attrData[0]  && this.attrNameInfo.length == '1' && this.shoppingCartJson.sale_id == '0' && this.shoppingCartJson.daily_id == '0'){
					this.$store.commit('alert',{show:true,text:this.chooseDescription});
					return;
				}
		...
		chooseAttribute : function(index,attrId, attrValueId, attrValue){
			...
			/****选择属性设置库存不足的属性不可选*****/
			if(attrId == 1 && this.preAttId == 1 && this.attrNameInfo.length == 2){
				this.checkNumber('',attrValueId);
			}
			if(attrId == 2 && this.preAttId == 2 && this.attrNameInfo.length == 2){
				this.checkNumber(attrValueId,'');
			}
			...
			}else if( this.attrNameInfo.length == 1){
				if( !this.attrData[0] && attrId == 1)
					this.chooseDescription = '请选择颜色';
				else if (!this.attrData[0] && attrId == 2)
					this.chooseDescription = '请选择规格';
				else if(this.attrData[0])
					this.chooseDescription = '已选' + this.attrValueData.join(',');
			}
